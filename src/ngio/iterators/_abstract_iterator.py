...
class AbstractIteratorBuilder(ABC, Generic[NumpyPipeType, DaskPipeType]):
    ...
    def by_chunks(
        ...
        Args:
            ...
            grid: `"write"` (default) sizes the tiles by the output image's
                write granularity — the shard shape when the output is sharded,
                the chunk shape otherwise — so the resulting ROIs pass
                `check_if_chunks_overlap`. Falls back to the input chunk grid
                when the iterator is read-only. `"read"` uses the input
                image's chunk grid.
