...
def test_by_chunks_grid_parameter():
    ...
    resulting ROIs pass the write-side gate; grid="read" restores input tiling."""
    ...
    # Coarser-chunked output: write tiling passes, read tiling correctly flags
    ...
    shard_tiled = iterator.by_chunks()
    ...
    # Read-only iterators fall back to the input grid
    ...
    assert len(feature_iterator.by_chunks().rois) == 16
