...
def test_on_disk_zoom_sharded_matches_unsharded(
    ...
    """Dask writes onto a sharded target are content-equal to an unsharded one.

    The dask blocks align to the target's inner chunks while write atomicity
    is per shard, so concurrent partial-shard writes race without the shared
    `da.store` lock. The unsharded layout is race-free by alignment and
    serves as the reference.
    """
    rng = np.random.default_rng(0)
    source_data = rng.integers(0, 255, size=(16, 128, 128), dtype="uint8")
    ...
    results = {}
    for name, shards in (("plain", None), ("sharded", (8, 32, 32))):
    ...
    np.testing.assert_array_equal(results["sharded"][...], results["plain"][...])
