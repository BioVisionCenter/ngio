...
- **Consolidation**: `mode="auto"` takes the 3–5x-faster in-memory path exactly where it is provably identical to the chunked one; `consolidate(regions=...)` (or `with image.track_writes():`) rebuilds only the pyramid regions that changed, byte-identical to a full rebuild — writing iterators use it automatically.
- **Config & plumbing**: `NgioConfig` gained a `zarr` section (`async_concurrency`, `threading_max_workers`); every `max_workers` accepts `"auto"`; `refresh()` on containers and plates re-reads all held metadata; the scheduling primitives (`plan_waves`, `canonical_unit_order`, `TailPolicy`, …) are public in `ngio.iterators`.
...
- **Tables and overlap checks**: type-filtered listings are one memoised pass (95 reads → 2 warm), ROI tables rebuild their DataFrame only on change, and the overlap checks sweep instead of scanning all pairs (2,048 ROIs: 1,083 ms → 31 ms).
