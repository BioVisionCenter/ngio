...
- **Iterators gained canonical `iter()`/`map()`/`reduce()` — numpy is the backend, so its name disappears from the method you type.** `map` and `reduce` carry the real implementations with full `max_workers`/`mapper` support; `iter_as_numpy`/`map_as_numpy`/`reduce_as_numpy` remain as permanent aliases, so nothing written against them ever breaks. `iter()` still resolves to dask this release (see the deprecation entry above for the migration).
