...
- **The dask iterator surface is deprecated; removal in `ngio=1.2`.** `iter_as_dask`, `map_as_dask`, `reduce_as_dask`, and `iter(data_mode="dask")` now emit an `NgioDeprecationWarning`. The two backends were never equals: nothing inside ngio calls the dask path, no documented example uses it, and this release's parallel mappers (`ThreadedMapper`, `ProcessMapper`) apply to the numpy path only — the dask methods hard-code the serial mapper because stacking a thread pool on dask's own scheduler oversubscribes rather than accelerates. The non-iterator dask API is untouched and staying: `get_as_dask`, dask-accepting `set_array`/`set_roi`, and pyramid consolidation (which defaults to dask) are load-bearing. For lazy whole-region access use `Image.get_as_dask`. At removal, `AbstractIteratorBuilder` collapses to a single pipe type parameter and the dask halves of the concrete iterators are deleted.
...
- **`Roi.compose` — a region's ROI plus a patch-local box gives the absolute ROI.** The recurring chore of every per-region flow: a function is handed the pixels of a region and finds something at local offsets, and someone must turn that into a world-anchored ROI — origin plus offset, scaled by the pixel size, with the padding-clipped-at-the-border subtlety that makes hand-rolling it a bug farm. `region_roi.compose(local_roi, pixel_size=...)` does it in one call: the local ROI is pixel-space, its starts counted from the region's first patch pixel; axes it does not pin inherit the region's extent (a 2D box found in one z-slab keeps the slab); `name`, `label` and extra fields ride through. The `space` fields make the classic frame mix-up loud: a world-space local (already absolute) and a pixel-space region both refuse. The detection iterator's own coordinate anchoring is this method — one implementation, used and pinned by both.
- **`ObjectDetectionIterator` — run a detector tile by tile, get one ROI table** ([#111](https://github.com/BioVisionCenter/ngio/issues/111)). A detector (a YOLO model, a maxima finder) answers in its tile's own pixels; `detect(func)` owns everything around that. Each tile reads a `padding` margin past its edge — the halo move, on a read-only iterator — so an object cut by a tile boundary is seen whole by at least one neighbour; `func(patch, roi)` — the `roi` identifying the (padded) region globally, the same shape of argument every iterator function receives — returns the boxes as a `DataFrame` or dict of columns (`x_min`/`x_max`/`y_min`/`y_max` bounding `[min, max)` in patch pixels, `z_*` for 3D, anything else — confidence, class — riding along into the table); the iterator anchors them into the reference image's world coordinates (`Roi.compose`) and resolves the duplicates the padding produces by standard greedy **non-maximum suppression** — boxes overlapping at or above `iou_threshold` are one object, the higher `score_column` (`"confidence"`; box size when unscored) survives, and per-tile NMS inside the detector composes cleanly with the cross-tile pass. Survivors come back renumbered `1..N` as a single `MaskingRoiTable`, ready for `add_table` — the iterator writes nothing. The per-tile map fans out under any `mapper` with no shared state (ids are derived from the tile index, not counted), a 2D detector on a 3D or timelapse image never has boxes merged across z or t, and the usual misconfigurations refuse loudly: mixed box dimensionality or scoring across tiles, inverted bounds, more detections than `block_size`, an `iou_threshold` of zero.
