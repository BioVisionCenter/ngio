...
**Highlights** — the iterators grew up in this release:

- **Parallel iteration**: `map`/`reduce` fan out on threads or processes via `mapper=ThreadedMapper(...)` / `ProcessMapper(...)`, lock-free by construction — ROIs sharing an output chunk are scheduled into conflict-free waves, so even masked iterators parallelize.
- **Batched inference**: `mapper=BatchedMapper(batch_size=...)` stacks patches into one `(B, ...)` array per `func` call — the fit for neural networks, ragged tiles padded and cropped back automatically. Or drive the loop yourself: `iter_batched(batch_size)` yields `(patches, writers)` batches on every iterator (payload lists on the read-only ones).
- **Halos**: `with_halo(x=, y=, ...)` reads a margin of context around each region and writes only the region — seamless tiles, unchanged parallelism.
- **Stitching**: `SegmentationIterator(..., stitch=True)` merges objects split across region boundaries and renumbers to a dense `1..N` — on any ROI list: grids with a halo, overlapping-FOV microscope layouts, ragged ROI tables.
- **Object detection**: the new `ObjectDetectionIterator` runs a detector tile by tile into a single deduplicated ROI table ([#111](https://github.com/BioVisionCenter/ngio/issues/111)).
- **One table from many regions**: `FeatureExtractorIterator.measure` joins per-region measurements into a single `FeatureTable`.

### Features

- **`BatchedMapper` — stack patches into one `(B, ...)` array per `func` call, for neural-network inference.** Per-call overhead dominates NN inference and the model wants batched input, so this mapper collects up to `batch_size` patches, stacks them behind a leading batch axis, and calls `func` once per batch — the one mapper that changes the func's contract (batch in, batch out; same leading axis). Tilings are ragged in general (`by_chunks` clips border tiles, halos shrink at image borders, masked regions are arbitrary), so each batch is padded to its per-axis maximum before stacking — origin-anchored, `pad_mode`/`pad_values` choosing how — and every shape-preserving output is sliced back to its region's true shape before the write; halos are trimmed after that, exactly as under any other mapper. Reads within a batch fan out on a thread pool (`read_workers`, they are round-trip bound); writes run serially on the calling thread, so batched mapping is write-safe on any tiling with no wave planning, and a `for_job` partition batches its own share. Tuple-unit iterators (feature extraction, object detection) refuse loudly — there is no single array to stack.
- The scheduling primitives are public: `plan_waves` (the conflict-free wave schedule), `canonical_unit_order` (the one write order every mapper uses), and the `TailPolicy` type joined `write_conflict_components` and `compute_write_footprint` in `ngio.iterators`.
- `iter_batched(batch_size)` iterates the ROIs in batches, for driving a model loop by hand: the writing iterators yield `(patches, writers)` — two aligned lists; stack, infer once, hand each result to its writer — and finalize on full drain, like `iter`; the read-only iterators yield their payload lists alone.
- **Distributed measurement and detection: the read-only iterators split too, via per-job *partials* and a single global merge.** Feature extraction and object detection end in a global join — one coalesce, one NMS pass — that per-job runs cannot reproduce piecewise (greedy NMS is not hierarchical: per-job suppression followed by a merge pass can keep different boxes than one global pass). So their distributed form stores each job's **raw pre-join records**: `iterator.for_job(**args).measure_to_partial(func)` / `.detect_to_partial(func)` write the measurements or the detector's pre-NMS boxes — tagged with their global ROI/tile index — into a transient `_ngio_partials` group beside the resolution levels, and `merge_partials()` rebuilds the full record set and runs the serial pipeline once, globally: **bit-identical to `measure` / `detect`, custom `coalesce` included** (it runs at merge time over the reconstructed per-ROI results; dicts are normalized to DataFrames and a `label` index to a `label` column). Partials go through ngio's own table machinery on the group handler — every store type and the retry policy inherited, the `tables` attribute never touched (one subgroup per job, no read-modify-write race, nothing registered) — and the merge deletes them on success; the final table is stored only by the caller's `add_table`, as ever. The guardrails mirror the writers': `prepare_jobs` is required (it wipes stale partials and stamps the plan fingerprint), a half-finished run refuses to merge instead of returning a silently incomplete table, the finished-table verbs refuse on a `for_job` slice, and a re-run job overwrites its own partial idempotently.
- **Distributed stitching and the three-phase recipe: `prepare_jobs(n_jobs)` is the init step of a distributed run — and with it, `stitch=True` distributes too.** Schedulers like Fractal run work as init → parallel tasks → consolidate; `prepare_jobs` is the init task: it performs the iterator's setup — always wiping stale scratch state from crashed or superseded runs first, so nothing corrupted can leak into a resolve — and returns the parallelization list as typed `JobArgs` dicts (JSON-serializable, splatting straight into `for_job(**args)`), with empty partitions already dropped so no task is submitted for a no-op. For plain writers the init step is optional; a **stitching segmentation** requires it, because the scratch root must exist race-free before any job banks into it. Each job creates and writes only its own tiles' bank arrays (idempotent on re-run; a failed job never destroys the others' banks), and the consolidate task's `finalize()` verifies every expected bank exists — a half-finished run errors, naming the tiles that never banked — then runs the one global resolve, relabels to a dense `1..N`, rebuilds the pyramid, and removes the scratch. Every step validates a **plan fingerprint** stamped at init (geometry, halo, stitch config, `n_jobs` — never ROI names, which are not process-stable), and every bank carries the run's id, so a drifted tiling or a stale bank from a superseded run fails loudly instead of resolving wrong. Banking claims nothing shared, so a stitched iterator splits into jobs exactly as the plain map would; note that *overlapping* tiles share label chunks and therefore always land in one job — which is precisely what keeps contested pixels deterministic across a distributed run.
- **Distributed runs: `iterator.for_job(job_index, n_jobs)` restricts a writing iterator to one of `n_jobs` independent partitions — for processes that cannot talk to each other, SLURM array tasks on a shared filesystem.** It is a builder call like `by_grid` or `with_halo` (applied last — reshaping a restricted iterator refuses): the returned iterator executes as usual, `map(func, mapper=...)` included, so each array task is `iterator.for_job(job_index, n_jobs=n_jobs).map(func)` with `job_index = int($SLURM_ARRAY_TASK_ID)`. Every job builds the identical iterator (construction is metadata-only and deterministic) and derives the same partition on its own; the guarantee is *embarrassing independence*: units are grouped by their write-conflict components — the same adjacency the wave scheduler uses, computed by the same code, so the two can never disagree — and **no write unit (chunk, or shard when sharded) is ever shared between two partitions**, so jobs need no locks, no barriers, and no channel to one another, in any order and any overlap in time. A restricted iterator's `map` deliberately does not finalize (and its `finalize()` refuses): after all jobs complete (a SLURM job dependency is the barrier) a final task builds the unrestricted iterator once more and calls `finalize()`, which consolidates only the union of what was written. Partitions are balanced greedily by unit count, surplus partitions beyond the component count come back empty and are no-ops, and the layout is inspectable — `partition_indices` on each slice, `write_conflict_components` (newly exported) for the grouping itself; effective parallelism equals the component count, which follows the *output's* chunking (a single-chunk output is one component by construction — a chunk is one atomic write object). Stitching segmentations and the read-only iterators distribute too, via the `prepare_jobs` init step and, for the read-only ones, the partial/merge verbs (see above).
- **Region-scoped consolidation: `consolidate(regions=...)` rebuilds only the pyramid regions that derive from what changed.** Pass the `Roi`s that were written — `set_roi(roi, patch)` then `consolidate(regions=[roi])`, the same vocabulary as the rest of the API; raw on-disk index tuples (what a setter pipe's `slicing_ops.normalized_slicing_tuple` produces) also work at every level, and `consolidate_pyramid` itself speaks only tuples, having no axes to interpret a `Roi` with. Or skip the bookkeeping entirely: `with image.track_writes() as regions:` records the region of every `set_*` call made through the handle inside the block — `set_array`, `set_roi`, the masked variants; nothing else, so writes through other handles, custom setter pipes, or worker processes are not seen — and the yielded list goes straight into `consolidate(regions=regions)`. Tracking nothing consolidates nothing: empty regions are a no-op, never a fallback to a full rebuild ("these are the touched regions: none" means there is nothing to rebuild). Only the chunks derived from the regions are read and rewritten per level, snapped outward to each level's write unit so every write has a single writer and the result is **byte-identical to a full rebuild** — the region path is a faster spelling of the same answer, never a different one. That guarantee holds exactly where `"auto"` already trusts the blockwise zoom (every edge an integral downsample, `order` not `"cubic"`); outside it, or once the merged regions cover more than `ConsolidationConfig.partial_max_coverage` of the source (default 0.5 — past that the bookkeeping costs more than it saves), the whole pyramid is rebuilt instead, silently. Touching regions merge into bounding boxes before any work happens, so a thousand adjacent ROI writes do not become a thousand rebuilds. **Writing iterators now use this automatically**: `finalize` derives the touched regions from the ROI list on the calling side — which is why it works under `ProcessMapper`, where nothing accumulated in a child would survive — so a `map` over a few objects of a large image consolidates a few chunk columns, not every level (the `consolidate_partial_dask` scenario pins a 80→32 chunk-read, 20→8 chunk-write drop against its full-rebuild control). A full-coverage iterator costs exactly what it did before, and stitched segmentations keep the full rebuild: the stitch resolve relabels level 0 globally, so only a full rebuild is guaranteed consistent after it.
- **`Roi.anchor` — a region's ROI plus a patch-local box gives the absolute ROI.** The recurring chore of every per-region flow: a function is handed the pixels of a region and finds something at local offsets, and someone must turn that into a world-anchored ROI — origin plus offset, scaled by the pixel size, with the halo-clipped-at-the-border subtlety that makes hand-rolling it a bug farm. `region_roi.anchor(local_roi, pixel_size=...)` does it in one call: the local ROI is pixel-space, its starts counted from the region's first patch pixel; axes it does not pin inherit the region's extent (a 2D box found in one z-slab keeps the slab); `name`, `label` and extra fields ride through. The `space` fields make the classic frame mix-up loud: a world-space local (already absolute) and a pixel-space region both refuse. The detection iterator's own coordinate anchoring is this method — one implementation, used and pinned by both.
- **`ObjectDetectionIterator` — run a detector tile by tile, get one ROI table** ([#111](https://github.com/BioVisionCenter/ngio/issues/111)). A detector (a YOLO model, a maxima finder) answers in its tile's own pixels; `detect(func)` owns everything around that. The iterator opts in to `with_halo` as a *pure read margin* — there is no write to crop it from — so an object cut by a tile boundary is seen whole by at least one neighbour; `func(patch)` returns the boxes as plain `Roi`s built the way any ROI is — `Roi.from_values(slices={"x": (x0, width), "y": (y0, height)}, name=None, space="pixel", confidence=0.9)` — pinning `x` and `y` (`z` optionally for 3D boxes) in patch pixels, every extra field — confidence, class — riding along into the table; the iterator anchors them into the reference image's world coordinates (`Roi.anchor`) and resolves the duplicates the halo produces by standard greedy **non-maximum suppression**, configured by `nms=NmsConfig(...)` exactly as stitching is by `stitch=StitchConfig(...)`: boxes overlapping at or above `iou_threshold` are one object, the higher `score_column` (`"confidence"`; box volume when unscored) survives, and per-tile NMS inside the detector composes cleanly with the cross-tile pass. Survivors come back renumbered `1..N` as a single `RoiTable` (name-indexed, the object id in the `label` column), ready for `add_table` — the iterator writes nothing. The per-tile map fans out under any `mapper` with no shared state, a 2D detector on a 3D or timelapse image never has boxes merged across z or t, and the usual misconfigurations refuse loudly: a world-space box (patch-local numbers in a world-labelled `Roi` would land it in the wrong place silently), a preset `name`/`label` (the iterator assigns them when renumbering — put a class label in an extra field, e.g. `class_id`), mixed box dimensionality or scoring across tiles, more detections than `NmsConfig.max_detections_per_tile`, an `iou_threshold` of zero.
- **`FeatureExtractorIterator.measure` — measure every region, get one table.** `measure(func)` runs `func(image, label, roi)` over every ROI and joins the per-region results into a single `FeatureTable` referencing the input label. The measurement fans out exactly like `reduce` (`mapper=ThreadedMapper(...)` or `ProcessMapper(...)` — per-region frames pickle back from worker processes), while the join happens once, at the end, on the calling thread. The iterator writes nothing: the returned table is stored by your own `add_table` call, which is where the name, backend and overwrite policy belong. `func` may return a `DataFrame` or the cheaper `dict[str, list]` of columns; either way the rows carry the object id in a `label` column, regions with no objects contribute nothing, and a custom `coalesce` takes over when the default join does not fit (any ngio table type can come out of it). The lazy `FeatureGetter` also stopped paying double IO: `get()` and the `.image`/`.label` properties now share one read per region instead of re-running it on every access.
- **Iterators gained canonical `iter()`/`map()`/`reduce()` — numpy is the backend, so its name disappears from the method you type.** `map` and `reduce` carry the real implementations, with a `mapper` argument as their only scheduling knob; `iter_as_numpy`/`map_as_numpy`/`reduce_as_numpy` remain as permanent aliases, so nothing written against them ever breaks. `iter()` still resolves to dask this release (see the deprecation entry above for the migration).
- **The transform contract shrank to two methods and one import.** A transform now implements `on_get(array, ctx)` (applied after reading) and `on_set(array, ctx)` (the inverse, applied before writing) — one generic `TransformProtocol` serving both backends, with the array type expressing which one; a transform supporting both dispatches on it, and a numpy-only transform is simply complete. The `ctx` is an `IoPipeContext` (importable as `TransformContext` too) — the pipe's own context object, built once per pipe — whose transform-stable fields carry the axis names, the slicing ops, and the ROI when there is one; so position-dependent transforms (an illumination correction sampling its flat-field profile at the patch's location, say) get what they need without reaching into pipe internals. Everything a transform author touches imports from one place: `ngio.transforms` exports `TransformProtocol`, `IoPipeContext`, `TransformContext`, `SlicingOps`, `AxesOps`, `ZoomTransform`, and `MaskTransform` — previously the protocol lived in `ngio.io_pipes` and the annotation types only in private modules. Passing a non-transform now fails at pipe construction with an `NgioValueError` naming it, and the appliers verify after each transform that the array is still the backend's type — which converts the worst failure mode, a transform silently materializing a dask array through `np.asarray` and destroying laziness while appearing to work, into a loud error naming the transform.
- **`ngio.transforms.UniqueLabelsTransform` — labels segmented region by region can no longer collide when they land in one array.** Each region is given a disjoint slice of the id space: `label.set_roi(roi, patch, transforms=[UniqueLabelsTransform(block_size, block_index)])` writes local ids `1, 2, 3` as `block_index * block_size + 1, +2, +3`, leaving background at `0`. Inside a masked iterator the ROI's own label supplies the block index, so nothing needs passing. The offset is **derived, not counted**, which is what makes it usable at scale: no shared state to synchronize, it works under `ProcessMapper` where a counter cannot, and it is idempotent — a region re-run after a failure produces exactly the ids it produced before, where a counter would produce a second set and leak the first. An offset that would not survive the patch's dtype raises rather than wrapping, naming the dtype and the block size (`derive_label` defaults to `uint32`, but a `uint16` label runs out fast). `on_get` subtracts the same offset, so a read through the transform hands back the region-local ids. It is an ordinary transform, so it composes with a merge: `transforms=[UniqueLabelsTransform(...)]` alongside `merge="max"` or `merge=MaskMerge(...)` does what it looks like.
- **A write can now combine with what is already on disk instead of overwriting it: `merge=` on `set_array`/`set_roi` and the setter pipes.** Pass a rule name — `"max"`, `"min"`, `"sum"`, `"keep_nonzero"` — a callable `(existing, patch, ctx) -> array`, or a policy object. `"max"`, `"min"` and `"sum"` are commutative and associative, so overlapping regions give the same answer whatever order they are written in; `"keep_nonzero"` is "the last nonzero write wins" and, like a custom rule, depends on the visit order. Deliberately no feathering: correct mean-blending needs an accumulated per-pixel weight, and the naive `w*patch + (1-w)*existing` darkens the first-written edge of every tile, so it would be a trap rather than a feature.
- **`Label.relabel_sequential()` renumbers a label's objects to a dense `1..N`.** Useful after anything that leaves gaps in the ids — a segmentation written region by region, a filter that dropped objects, or a stitch run with `compact=False`. Numbers are handed out in **first-encounter order over the chunk grid**, not by sorting the existing ids, and that is what makes it a single pass: a sorted numbering cannot be decided until every id has been seen, so it needs one traversal to collect and another to write. The result is unique and gapless either way; only *which* object becomes `1` differs, and it now follows the array rather than whatever offsets happened to be assigned. Deterministic for a given chunking. The pyramid is rebuilt afterwards, since every level derives from level 0.
- **Segmentation iterators can stitch: `SegmentationIterator(..., stitch=True)` resolves objects split by a tile boundary into one id.** Each tile is segmented independently and numbers its objects from 1, so without stitching two objects in two tiles are both `1` and nothing downstream can tell them apart; the map phase now offsets each tile's ids into a block of its own, and a pass afterwards merges the ids that turn out to name the same object.

  **Any ROI list stitches** — a regular grid with a halo, an overlapping-FOV microscope layout (with or without a halo: the FOV overlap itself is the evidence), a ragged ROI table. `MaskedSegmentationIterator` takes `stitch=True` too, for tiling *within* a mask: sub-objects split by a tile boundary inside one masked object merge into one id, each tile banks only what its own mask can write, tiles of different masks are never compared, and ids come out unique and dense across every object — `UniqueLabelsTransform` is no longer needed on this path (combining it with `stitch` raises, on both segmentation iterators: each offsets the ids, and doubly-offset labels would corrupt the resolve). The evidence is overlap between two independent predictions, not adjacency across the cut, so two objects that merely touch stay distinct. Each tile banks its full (id-offset) prediction into its own transient scratch array, with its position in the attrs — conflict-free in any topology — and the resolve compares every pair of banks that overlap in pixel space (IoU ≥ `iou_threshold`, default `0.3`, erring towards leaving an object split), unions the agreeing ids, and compacts to a dense `1..N` unless `compact=False`. Where two tiles wrote the same output pixels and their objects did not match, the later write wins — deterministically, because every mapper runs in the same wave order. `StitchConfig(scratch_store=...)` puts the banks anywhere (a `MemoryStore` leaves the output store untouched; `ProcessMapper` refuses it, since it pickles by value). Tiles that touch across an axis with no halo warn: those seams stay unstitched. A run interrupted between the map and the resolve leaves a valid, merely over-split segmentation, and the resolve is idempotent.
- **Iterators gained `with_halo(x=, y=, z=, t=)`: read a margin of context around each ROI, write only the ROI.** The function is handed the grown region and must return it grown too; the border is cropped off before the write. That is the fix for seam artifacts — a smoothing or a segmentation sees past its own tile edge — and it is also "trim", which is the same operation viewed from the other side. Crucially the ROIs do not move, so **write footprints are unchanged and a haloed iterator parallelizes exactly as far as it did without one**: overlapping *writes* would have to be serialized, overlapping reads cost nothing. Margins are in reference-image pixels and clip at the borders, so an edge tile grows only on the sides where there is room and the crop is asymmetric to match. The crop is a setter wrapper rather than a transform: reading a bigger region than you write is a slicing asymmetry, not a mapping of the patch, so it was never chain material. Returning a core-sized array raises rather than writing a shifted tile. Read-only iterators refuse the call — with no written region there is nothing for a halo to surround — unless they explicitly opt in to the halo as a pure read margin whose overlap they reconcile themselves, as the detection iterator does with NMS. So does a **shape-changing transform on the data path** — a `ZoomTransform` round-trip (downsample for the model, upsample to write) works fine on its own, but its patch lives on a rescaled grid where the reference-pixel margins no longer describe the border, and `ZoomTransform`'s inverse tolerates a pixel of slack, so a crop at the wrong scale would be quietly resampled into place rather than caught. Refused rather than guessed at; iterate on a coarser pyramid level to work at a lower resolution. Masking is unaffected and composes with a halo normally — its zoom rescales the *label* it reads, never the data array.
- **Combining with the destination is a `merge=`, not a transform — and the distinction is what keeps the transform chain rule-free.** A transform is a function of the patch alone, so the chain composes and inverts with no constraints on order, count, or position. A merge is a function of the patch *and* the destination, and has no coherent slot inside a fold of mappings; it runs once, after the chain. Placing it there also fixes *where* it happens: the patch has been through every inverse, so it is in the array's own space, and the destination is read raw with no chain replayed over it. Both sides are in the same space by construction. That is stronger than merging in user space, which is what the released masked setters did: with a lossy transform in the chain, the pixels a mask *promises to protect* used to be written back as `T_set(T_get(disk))` rather than carried through untouched, and the order-independence of `"max"`/`"min"`/`"sum"` was only approximate across repeated overlapping writes. Both are now exact. For the common case — no transforms, or per-pixel ones — the result is unchanged.
- **Masking split into its two actual operations.** On a read it replaces outside-mask pixels with a fill value, which is a function of the patch alone: `ngio.transforms.MaskTransform`, an ordinary transform with no position rule. On a write it keeps the on-disk pixels outside the mask, which depends on the destination: `ngio.transforms.MaskMerge`, passed as `merge=`. The single class that did both is gone; putting `MaskTransform` on a write raises and names `MaskMerge`, because filling has no inverse and silently doing nothing would be worse. Both take the same arguments and select the same way — `label == roi.label`, or `label != 0` for an unlabelled ROI — so one instance of each serves every ROI, and a label at a different pyramid level is still rescaled to the data grid (nearest) unless `allow_rescaling=False`. The high-level API is untouched: `get_roi_masked` and `set_roi_masked` pick the right one for you.
- **`MaskTransform` replaces the four masked pipe classes.** One backend-dispatching implementation replaces the numpy/dask duplication, and masking composes with everything the pipes already do: `image.get_roi(roi, transforms=[MaskTransform(label=nuclei, target_image=image)])` reads a masked patch, and `image.set_roi(roi, patch, merge=MaskMerge(label=nuclei, target_image=image))` writes one without disturbing the pixels outside. See the two entries below for why those are separate objects.
...
- **Iterators can map in parallel: `map(func, mapper=...)`, `ThreadedMapper`, and `ProcessMapper`.** `mapper=ThreadedMapper("auto")` (or an int) fans the per-ROI read → `func` → write units out on a thread pool — the fit for round-trip-bound IO and GIL-releasing compute; `mapper=ProcessMapper(...)` fans out on a spawn-based process pool for pure-Python `func`s, where each child reads from and writes to the store directly so pixels never cross the process boundary. A mapper is the *only* way to ask for concurrency here: the pool size lives on the mapper that owns it, rather than being spelled a second time at the call site as a `max_workers` argument on `map`/`reduce` — which would then need a rule for what happens when both are given. (The `max_workers` argument on the *plate* operations is a different thing and is unaffected.) Before any fan-out both mappers compute every ROI's write footprint on the output array and plan the units into **conflict-free waves** (greedy first-fit coloring of the write-footprint conflict graph): two ROIs sharing a write unit never run in the same wave, and the waves run back to back on one pool, each at full width. Within a wave disjointness is the entire safety argument, and it holds in any topology where a lock could only ever protect one process (the recorded failure of the old `DASK_STORE_LOCK`). A conflict-free tiling — `by_write_units()` tiles on the output's write grid — runs as a single fully-parallel wave, and that is what makes masked iterators parallelize out of the box: per-object bounding boxes routinely share chunks even when the boxes do not overlap, and the wave count stays around the worst chunk's object multiplicity rather than the ROI count. A schedule that degrades to fully serial is logged as a warning rather than silently absorbed; `require_no_write_units_overlap()` remains the strict opt-in gate for insisting on a single wave. The default (`mapper=None`) stays serial, permanently — parallel *writes* remain explicit opt-in, unlike the plate-read fan-outs whose default flips to `"auto"` in 1.2 — and the op-count gate pins the parallel map to a byte-identical tally with the serial one while the concurrency gate pins that all four units genuinely overlap. `ProcessMapper` refuses in-memory stores (a `MemoryStore` pickles by value, so children would write into private copies) and requires a picklable `func`. The shared `max_workers` machinery moved from `images/_table_ops.py` to `ngio.common._concurrency`, unchanged.
- **Iterators gained `reduce`**: apply a function to every ROI and collect the results in ROI order, writing nothing — the natural shape of per-ROI measurement, which previously had to go through a writing `map` or a hand-rolled loop. Units are built read-only even on writable iterators, and `finalize` does not run.
- **The tiling calls say what they tile by.** `by_grid(size_x=..., tail=...)` lays a regular grid, and its `tail=` policy decides what happens where the grid does not divide the axis: `"clip"` (the default) shrinks the last tile to the border; `"balance"` re-splits the last two tiles so a thin overhang never yields a thin tile — 100 px at 32 gives `32, 32, 18, 18` rather than `32, 32, 32, 4` (adjacent tiles only: with overlap there is no well-defined "last two"); `"shift"` slides the last tile back to stay full-size, the sliding-window trick for fixed-input models (it then overlaps its neighbour, which a writing iterator must reconcile with a merge or serial execution); `"drop"` discards it. `by_blocks(num_x=..., num_y=...)` is the complement — how many tiles, not how big — with a balanced partition and no tail by construction. `by_chunks()` tiles by the *input* image's chunk grid, the unit of reading; `by_write_units()` tiles by the *output*'s write granularity — the shard shape when the output is sharded (writes are atomic per shard object), the chunk shape otherwise, via the new `AbstractImage.write_granularity` — so the resulting ROIs are collision-free on the write target by construction, falling back to the input grid for read-only iterators. Both storage tilings pin `tail="clip"`: shifting or balancing tiles would misalign them from the storage grid and defeat the point.
...
- `FeatureExtractorIterator` units no longer retain their patches after being consumed, so `reduce`/`measure` memory stays one patch deep instead of holding the whole image.
- `ZarrGroupHandler.get_handler(..., overwrite=True)` now recreates the group instead of serving the cached handler, and `delete_group` evicts the deleted subtree from every cache — a second distributed `prepare_jobs → merge_partials` cycle in one process now works.
- The dask iterator verbs (`map_as_dask`/`reduce_as_dask`) reject parallel mappers: `store_dask` scopes a process-global dask config option, so concurrent dask writers were unsafe.
- Stitching overlapping or ragged ROI lists used to corrupt the shared scratch silently (colliding bands, wrong seam pairs); the per-tile banks make both configurations first-class (see Features).
- Stitching world-space ROI tables whose pixel origins are not integers (an FOV table at a real pixel size) and border-overrunning ROIs now works: the bank shapes and the halo margins are derived from the same floor/ceil-and-clamp normalization the setters use, instead of truncating on their own.
- A stitched `finalize()` with a drifted plan no longer wipes the jobs' banked predictions before erroring — it refuses to touch them; only `prepare_jobs` deliberately starts over. A second `finalize()` after a completed run raises "nothing to resolve" without creating (and leaving behind) a fresh scratch group.
- A wiped stitch scratch reads back as *missing* instead of as silent zeros: the per-bank staleness check now runs on every open (the handle cache in front of it defeated the `run_id` guard).
- Pickled stitch plans no longer ship the full tile geometry with every unit — `ProcessMapper` IPC for a stitched map scaled as tiles × pairs.
- A haloed *writing* iterator refuses `reduce` and read-only `iter`: with no write to crop the halo from, they silently measured the grown regions. (Masked iterators are unaffected: overlapping bounding boxes with disjoint pixels — organoids — remain fully supported, now pinned by a test.)
- `map` on a read-only iterator refuses even with an empty ROI list (it slipped past the writability guard and silently did nothing).
- `with_halo` refuses in-place iteration (same array as input and output): a grown read covers pixels neighbouring tiles write, so results depended on execution order.
- Distributed runs no longer mistake an unreadable partial or scratch group for an absent one: only "not found" means "not run yet"; any other read failure raises instead of silently re-running or wiping banked state.
- `ObjectDetectionIterator.merge_partials` no longer attaches `NaN` extra fields to boxes when tiles report heterogeneous extras (an artifact of the partial tables' column union).
- `relabel` keeps its id lookup in the label's own dtype, so `uint64` ids stay exact (the `int64` keys promoted the comparison to `float64`) and an out-of-range mapped value now raises instead of wrapping.
- `ObjectDetectionIterator` gained the deprecated `iter_as_dask` shim its sibling read-only iterator already had, so it deprecates instead of erroring until the 1.2 removal.
- **Metadata updates no longer drop what they were not about** ([#231](https://github.com/BioVisionCenter/ngio/issues/231)). `set_axes_units` on an ngff 0.5 container silently reset the OMERO channel labels to `channel_0, channel_1, …`, and the same held for `set_axes_names`; on a label, the image-label source metadata was silently reset to its default. The root cause was one bug: the meta objects' reshaping methods (`to_units`, `rename_axes`, `rename_image`) rebuilt the object from `(version, name, datasets)` alone, dropping any subclass state — an image's channels, a label's source. Reconstruction now goes through a hook each subclass completes with its own state, and updates preserve channels and sources on both ngff 0.4 and 0.5. (0.4 only ever *looked* immune: its encoder happened to leave the stale `omero` attributes in place.)
- **`stitch=True` now refuses the dask write path instead of corrupting the labels.** The dask setters never applied the stitch wrapper, so a `map_as_dask` or `iter(data_mode="dask")` run on a stitching iterator wrote un-offset tile-local ids and banked no halo bands — and the resolve afterwards compacted those colliding ids into a silently wrong labelling. Stitching is a numpy-path feature; the dask setter now says so with an `NgioValueError`.
- **The stitch plan validates its inputs.** A duplicate ROI name silently collapsed two tiles into one record — it now raises at plan build. Tiles split along an axis with no halo used to silently shadow each other; they now run with a warning that seams along that axis stay unstitched. A tile count that would overflow the label dtype (`(n+1)·block_size`) raises at plan build instead of at tile N's write.
- **A merge can no longer broadcast a wrong-shaped patch or hand zarr a promoted dtype.** Without a merge, a `(1, Y, X)` patch into a 3-channel region is a shape error; with one, `np.maximum` and friends broadcast it into a correctly-shaped result and the write check waved it through — the patch silently replicated across every channel. The patch is now checked against the destination before the rules touch it, and the merged result must keep the destination's shape *and* dtype: a float patch under `merge="max"` promoted the result to float64, which zarr then wrap-cast back into the integer array (`-1.0` became `65535`). `merge="sum"` on matching integer dtypes can still wrap within the dtype — that is numpy addition, documented rather than detectable without an upcast.
- **`UniqueLabelsTransform` checks dask patches too, and both offset paths refuse block spill.** The overflow guard ran only on numpy patches; a dask patch whose shifted ids exceeded the dtype wrapped silently (the offset itself fitting, e.g. `60000 + 9999` in uint16). The dask path now validates each block as it materializes. And an id at or above `block_size` — which lands in the *next* region's block and collides with its labels — was never checked on either path (only dtype overflow was); both `UniqueLabelsTransform` and the stitch offsets now refuse it, on the same reasoning as the dtype check: the collision is invisible in the output.
- **A dropped channel selection is no longer validated first.** With `remove_channel_selection=True` on an array with no channel axis, a `c=slice(...)` entry hit the virtual-axes validation *before* the removal it was exempted by, and raised. The removal now precedes the validation.
- **The zoom transform's edge cases are pinned down.** The forward zoom shape is derived from the raw slice on purpose — the raw slice carries the ROI's sub-pixel world bounds, which is what makes a zoomed patch land exactly on the target grid — but raw did not mean clamped, and three edges fell through: a slice reaching past the array's *start* (`y=slice(-5, ...)`) counted pixels the clamped read never delivered and stretched the patch; a zero-length or entirely-past-the-end slice reached the zoom kernel with a zero (or negative) extent and died in a divide-by-zero reshape; and a non-contiguous list selection on a scaled axis was silently stretched as if its elements were adjacent — `y=[0, 2, 7]` at 4x came back as 12 rows resampled across pixels that were never neighbours. Now: negative starts are clamped like the read, an empty selection zooms to an empty patch on both backends, and a scaled list selection raises. The module now documents the interlocking invariants (raw-forward vs normalized-inverse, and why) at the top, and every one of them is pinned by a test; the zoom's shape errors are `NgioValueError` throughout.
- **Writing through a chain of transforms now inverts the chain.** The set-side appliers iterated in the same order as the get side: for a chain `[A, B]` a read is `B(A(x))`, so a write must apply `A⁻¹(B⁻¹(y))` — but it applied `B⁻¹(A⁻¹(y))`. Harmless for the single-transform chains ngio itself builds (the masked pipes' injected zoom is get-side only), silently wrong for any user chain of two or more transforms on a write path.
- **ngio no longer warns you to pass a `mode` you cannot pass.** The `consolidate` deprecation notice fired from internal call sites — `create_ome_zarr_from_array`, `create_synthetic_ome_zarr`, and the writing iterators' `finalize()` — pointing at the caller's frame with advice about a kwarg those functions did not expose. Creation and the iterators now expose `consolidation_mode` and forward it, so the notice is actionable where it appears; the synthetic helper pins `mode="dask"` and stays silent. The gap was invisible in CI because the test configuration ignores `NgioFutureWarning` globally.
...
- Serial `map` over ROIs with overlapping write footprints now runs in wave order (the parallel mappers' schedule) instead of ROI order, so "who wrote last" on a contested pixel is identical under every mapper. Disjoint tilings are unaffected — with no conflicts the wave order is the ROI order.
- `by_grid(tail="drop")` raises when it would drop every tile along an axis (axis shorter than the tile size) instead of silently iterating nothing.
- `SegmentationIterator.map` takes `mapper` keyword-only, matching the base class and every other iterator verb.
- **The iterator surface was normalized for its promotion out of `experimental`** — one-time renames, no deprecation shims, since the 1.0 surface was explicitly experimental: `grid()` → `by_grid()` (the word "grid" now has exactly one meaning on the class), `by_chunks(grid="write")` → `by_write_units()` (and `by_chunks()` keeps the input-chunk meaning its name always claimed), `overlap_xy` → per-axis `overlap_x`/`overlap_y`, `check_if_chunks_overlap`/`require_no_chunks_overlap` → `check_if_write_units_overlap`/`require_no_write_units_overlap` (they measure shard-or-chunk write units, not chunks), `post_consolidate()` → `finalize()`, and `ImageProcessingIterator(input_channel_selection=)` → `channel_selection=` under the documented convention that a bare parameter names the input side and output-side parameters are always prefixed.
- **A merge is stricter about what it writes** (see Fixes): the patch must match the destination region's shape exactly, and the merged result must keep the destination's dtype — cast your patch before writing. Building a *getter* with `merge=` now raises instead of silently ignoring it.
- **`max_workers` is validated everywhere it is accepted**: `0` or a negative — previously clamped to 1 on the plate operations, i.e. silently serial under the flag that asked for parallelism — now raises on the parallel mappers and the plate/table fan-outs alike.
- **A writing `map` no longer holds every written patch until it returns.** `BasicMapper` and `ThreadedMapper` collected each tile's result — for a writing map, the full written output, alive in memory at once — and `map` then discarded the list. Written units now collect `None`, exactly as `ProcessMapper` always did; `reduce` results are unchanged. Third-party mappers keep their contract, with the protocol note that written units' results may be `None`.
- **A failed stitching `map` deletes its scratch group.** A mapper exception used to leave the transient `_ngio_stitch` group sitting beside the resolution levels. The tiles already written stay (re-running the map reproduces the same ids — the offsets are derived, not counted), but the scratch is removed.
- **`MaskedSegmentationIterator` validates input/output dimensions again**, matching `SegmentationIterator`; the check had been commented out. `by_zyx(strict=True)` and the pipes' missing-ROI accessor now raise `NgioValueError` (both subclass `ValueError`, so existing handlers keep working).
...
- **`check_if_write_units_overlap` (formerly `check_if_chunks_overlap`) measures the write target, at write granularity.** It used to build chunk sets from the *input* image's chunk grid — but writes land on the output, whose chunking can differ, and on a sharded output the atomic unit is the shard, not the chunk. It now computes each ROI's write footprint from the setters (`ChunkRect`, exact for slice/int selections) against `shards or chunks` of the actual output array; a read-only iterator has no write hazard and returns `False`. Two regressions the old check missed are now pinned by tests: ROIs disjoint on the input grid but sharing a coarser output chunk, and ROIs disjoint at inner-chunk granularity but sharing one shard.

### Deprecated

- **`set_axes_units` is deprecated in favour of `set_space_unit` / `set_time_unit`** ([#232](https://github.com/BioVisionCenter/ngio/issues/232)). The batch form sets *both* units on every call — an omitted parameter is set to its default, not left unchanged — which reads as a bug the moment you call `set_axes_units(space_unit="micrometer")` on an image whose time unit was `"hour"` and find it back at `"second"`. That behaviour is kept (and now documented on the method), but the split methods say what they mean: each touches exactly one unit and leaves the other alone. Available on images, labels, the images container and `OmeZarrContainer` (where `set_labels=True` carries the unit into the labels). Removal of the batch form in `ngio=1.2`.
- **The `ngio.experimental.iterators` alias now says `ngio=1.2`.** It was scheduled for removal in 1.1, but it is still imported across the Fractal tasks ecosystem, so it gets one more release of forwarding (with its warning) while those imports move to `ngio.iterators`. It is the only 1.1-scheduled removal that slips — see Removed.
- **The dask iterator surface is deprecated; removal in `ngio=1.2`.** `iter_as_dask`, `map_as_dask`, `reduce_as_dask`, and `iter(data_mode="dask")` now emit an `NgioDeprecationWarning`. The two backends were never equals: nothing inside ngio calls the dask path, no documented example uses it, and this release's parallel mappers (`ThreadedMapper`, `ProcessMapper`) apply to the numpy path only — the dask methods default to the serial mapper because stacking a thread pool on dask's own scheduler oversubscribes rather than accelerates. The non-iterator dask API is untouched and staying: `get_as_dask`, dask-accepting `set_array`/`set_roi`, and pyramid consolidation (which defaults to dask) are load-bearing. For lazy whole-region access use `Image.get_as_dask`. At removal, `AbstractIteratorBuilder` collapses to a single pipe type parameter and the dask halves of the concrete iterators are deleted.
- **`iter()` without a `data_mode` will yield numpy arrays in `ngio=1.2`** — today it defaults to dask, an accident of history that every documented example already works around by calling `iter_as_numpy()`. A bare `iter()` now emits an `NgioFutureWarning`; pass `data_mode="numpy"` (or use `iter_as_numpy()`) to adopt the future behaviour now, or `data_mode="dask"` to pin the current one. `FutureWarning` rather than `DeprecationWarning` for the same audience reason as the `max_workers` entry below: the bare caller did not pick a backend and would otherwise get a silent result-type change.
- **The legacy transform contract (`get_as_numpy_transform`/`set_as_numpy_transform`/`get_as_dask_transform`/`set_as_dask_transform` methods) is deprecated; removal in `ngio=1.2`.** Old-style transforms keep working — the pipes wrap them in an adapter at construction and emit an `NgioDeprecationWarning` naming the class — with unchanged behaviour, including a dask method that raises `NotImplementedError` continuing to do exactly that. Implement the new two-method contract instead (see the Features entry below); for a transform whose dask methods were passthroughs or `NotImplementedError` the migration is a net deletion of code.
- **The ROI and masked pipe classes are deprecated; removal in `ngio=1.2`.** The io pipes had grown three parallel families where one suffices: a ROI pipe was a bare pipe that converted its ROI to slices, and a masked pipe was a ROI pipe that also read a label — both now expressible as arguments to the four bare pipes. `NumpyRoiGetter`, `DaskRoiGetter`, `NumpyRoiSetter` and `DaskRoiSetter` become `NumpyGetter(..., roi=...)` and friends — the `roi` argument the bare pipes already accepted now slices, with explicit `slicing_dict` entries still overriding the ROI per axis. `NumpyGetterMasked` and `DaskGetterMasked` become a bare pipe with `ngio.transforms.MaskTransform` in the transforms list; `NumpySetterMasked` and `DaskSetterMasked` become a bare pipe with `merge=ngio.transforms.MaskMerge(...)`. All eight names keep working through 1.1 as thin shells over the replacements and emit an `NgioDeprecationWarning`; the high-level API (`get_roi`, `get_roi_masked`, iterators) is untouched. Three observable details moved with the mechanics: a masked *getter* shell's `.transforms` now ends with the mask transform while a masked *setter* shell carries it on `.merge` instead, the shells are now instances of the bare pipe classes, and an invalid ROI against the label surfaces at the first read or write instead of at shell construction (same exception, later moment).
...
- **The surfaces v1.0.0 deprecated for removal in `ngio=1.1` are gone** (except the `experimental.iterators` alias — see Deprecated): the `conctatenate_tables` typo alias, the `set_axes_unit` singular alias, the `levels_paths=` and `validate_paths=` keyword aliases, and the `*_async` variants (`get_images_async`, `get_wells_async`, `images_paths_async`, `list_image_tables_async`, `concatenate_image_tables_async`, `concatenate_image_tables_as_async` — on the plate and as free functions). Every one of them warned with this removal version throughout 1.0.x, and the synchronous equivalents (`max_workers=` for the fan-outs) are drop-in: `plate.get_images_async()` becomes `plate.get_images(max_workers="auto")`.

### Performance

- **Consolidating a sharded pyramid no longer reads the source one inner chunk at a time.** The dask and coarsen paths read the source at inner-chunk granularity, and on a sharded array each of those blocks is a *partial-shard* read — a shard-index fetch plus a range request. The source is now read at its write unit (one coarse fetch per shard) and immediately rechunked back to the inner chunks in memory, which is pure views and — load-bearing, not cosmetic — keeps the zoom's block grid, and therefore every output pixel, byte-identical (the sharded geometry matrix pins it). `consolidate_sharded_dask`: **324 chunk reads (320 partial) → 8 (0 partial)**, bytes read −9%, writes unchanged; on a remote store each avoided partial get is a round-trip. Unsharded pyramids are byte-for-byte unchanged. The ROI read path (`get_slice_as_dask`) deliberately keeps the inner-chunk grid — a ROI smaller than a shard would otherwise fetch the whole shard to serve a fraction of it — and now carries a comment saying so.
...
- **The overlap checks are no longer quadratic.** `check_if_regions_overlap` sorts on the most discriminating axis and sweeps, comparing each region only against those still open — 2,048 disjoint ROIs drop from **1,083 ms to 31 ms**, and 8,192 finish in 0.3 s where the all-pairs scan would need ~17 s. the write-units overlap check now accumulates one seen-set instead of intersecting every pair. List selections and stepped slices, which are not intervals, keep the pairwise fallback.
...
- The performance gate gained an `iterator_map_numpy` scenario: a writing iterator end to end, recording per-ROI reads and writes, the per-ROI metadata probes, and `finalize`'s whole-pyramid rebuild — the iterator knows exactly which regions it wrote and rebuilds every level anyway. A future region-scoped consolidation lands here as a committed `get.chunk`/`set.chunk` drop.
