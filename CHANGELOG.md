...
- **Batched inference**: `mapper=BatchedMapper(batch_size=...)` stacks patches into one `(B, ...)` array per `func` call — the fit for neural networks, ragged tiles padded and cropped back automatically.
...
- **Stitching**: `SegmentationIterator(..., stitch=True)` merges objects split by tile boundaries and renumbers to a dense `1..N`.
...
- **One table from many regions**: `FeatureExtractorIterator.reduce_to_table` joins per-region measurements into a single `FeatureTable`.
...
- **`FeatureExtractorIterator.reduce_to_table` — measure every region, get one table.** `reduce_to_table(func)` runs `func(image, label, roi)` over every ROI and joins the per-region results into a single `FeatureTable` referencing the input label. The measurement fans out exactly like `reduce` (`mapper=ThreadedMapper(...)` or `ProcessMapper(...)` — per-region frames pickle back from worker processes), while the join happens once, at the end, on the calling thread. The iterator writes nothing: the returned table is stored by your own `add_table` call, which is where the name, backend and overwrite policy belong. `func` may return a `DataFrame` or the cheaper `dict[str, list]` of columns; either way the rows carry the object id in a `label` column, regions with no objects contribute nothing, and a custom `coalesce` takes over when the default join does not fit (any ngio table type can come out of it). The lazy `FeatureGetter` also stopped paying double IO: `get()` and the `.image`/`.label` properties now share one read per region instead of re-running it on every access.
...
- **Segmentation iterators can stitch: `SegmentationIterator(..., stitch=True)` resolves objects split by a tile boundary into one id.** Each tile is segmented independently and numbers its objects from 1, so without stitching two objects in two tiles are both `1` and nothing downstream can tell them apart; the map phase now offsets each tile's ids into a block of its own, and a pass afterwards merges the ids that turn out to name the same object.
  ...
  The evidence is **overlap between two independent predictions, not adjacency across the cut** — which is the whole reason it requires a halo (it raises without one). Two objects that merely touch at the boundary are adjacent but do not overlap, so an adjacency rule merges them and an overlap rule does not; each tile reads past its own edge, so it has an opinion about the strip its neighbour owns, and the two opinions are matched one-to-one. Those opinions have to be *kept* — a tile writes only its core, so the halo prediction is otherwise discarded — which is what the transient scratch arrays are for: one per haloed axis, holding the forward band each tile predicts inside the next tile's core, removed once the stitch resolves. `StitchConfig(scratch_store=...)` puts them anywhere you like — a `MemoryStore` is often the right answer, since labels compress well and the bands are small, and it leaves the output store untouched; `ProcessMapper` refuses an in-memory scratch, because a `MemoryStore` pickles by value and each worker would bank its bands into a private copy. Bands are collision-free by construction rather than by checking, because a tile writes inside exactly one neighbour and the band is clipped to it.

  Write footprints on the label are unchanged, so **stitching parallelizes exactly as far as the map did** — pinned by a test that serial and threaded runs produce identical labellings. `iou_threshold` (default `0.3`) sets how much two tiles must agree before their ids are joined; it errs towards leaving an object split, which is fixable downstream, rather than merging two that are not. Ids are compacted to a dense `1..N` unless `compact=False`, by delegating to `Label.relabel_sequential` — so the compaction happens in the same pass as the relabel rather than costing a second read of the label, and the numbering follows the array rather than the tile blocks. Tile grid positions come from *ranking* the distinct ROI origins rather than dividing an origin by a tile size — `by_grid()` clips the final tile against the parent ROI, so a 100-pixel axis tiled at 32 ends `start=96, length=4`, where `96 // 4` gives 24 and the answer is 3. A run interrupted between the map and the resolve leaves a valid, merely over-split segmentation, and the resolve is idempotent.
...
- **The stitch plan validates its tile grid.** Two tiles that differ only along an axis with no halo land on the same grid cell over the haloed axes; the neighbour map is keyed by cell, so one silently shadowed the other and whole seams went unstitched — the 3D case "tiled in z, haloed in y and x" produced quietly wrong merges. The plan is also keyed by ROI name, where a duplicate silently collapsed two tiles into one record. Both now raise at plan build, naming the offending tiles.
