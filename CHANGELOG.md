...
- **`ObjectDetectionIterator` — run a detector tile by tile, get one ROI table** ([#111](https://github.com/BioVisionCenter/ngio/issues/111)). A detector (a YOLO model, a maxima finder) answers in its tile's own pixels; `detect(func)` owns everything around that. The iterator opts in to `with_halo` as a *pure read margin* — there is no write to crop it from — so an object cut by a tile boundary is seen whole by at least one neighbour; `func(patch, roi)` — the `roi` identifying the (haloed) region globally, the same shape of argument every iterator function receives — returns the boxes as a `DataFrame` or dict of columns (`x_min`/`x_max`/`y_min`/`y_max` bounding `[min, max)` in patch pixels, `z_*` for 3D, anything else — confidence, class — riding along into the table, except the reserved `name`/`label`/`slices`/`space`, which must be renamed, e.g. a class `label` to `class_id`); the iterator anchors them into the reference image's world coordinates (`Roi.anchor`) and resolves the duplicates the halo produces by standard greedy **non-maximum suppression**, configured by `nms=NmsConfig(...)` exactly as stitching is by `stitch=StitchConfig(...)`: boxes overlapping at or above `iou_threshold` are one object, the higher `score_column` (`"confidence"`; box volume when unscored) survives, and per-tile NMS inside the detector composes cleanly with the cross-tile pass. Survivors come back renumbered `1..N` as a single `RoiTable` (name-indexed, the object id in the `label` column), ready for `add_table` — the iterator writes nothing. The per-tile map fans out under any `mapper` with no shared state, a 2D detector on a 3D or timelapse image never has boxes merged across z or t, and the usual misconfigurations refuse loudly: mixed box dimensionality or scoring across tiles, inverted bounds, more detections than `NmsConfig.max_detections_per_tile`, an `iou_threshold` of zero.
