...
- Added performance measurement, split in two because the halves are different kinds of thing. `tests/performance/` asserts exact store-operation counts against committed baselines and runs in CI like any other test: ngio's regressions are algorithmic, so they are exact integers with zero variance, and a change that multiplies store reads fails the build with a readable before/after diff. Counts are backend-independent, so a local measurement predicts the cost on S3 without any credentials in CI — checked rather than assumed, by running every scenario against both a local and an in-memory store. `benchmarks/` is a standalone suite reporting wall clock *and peak memory* for the questions counts cannot answer, such as which pyramid consolidation mode fits in RAM. There is no history file: the baselines are committed JSON, so `git log -p` on them already is the history.
- `benchmarks/` (`python -m benchmarks`) is organised as one file per measured block, each declaring its **axes** as data — the runner measures every point of their cartesian product. Sweeps are therefore reachable from the command line rather than being literals in a loop body: `--axis consolidate.z=16,1024` extends a sweep past what the block declares, `--list` prints the whole sweepable surface, and each axis value lands in its own CSV column so a result file pivots without parsing label strings. Blocks are imported lazily and know nothing about timing; the CLI knows nothing about ngio.
- `benchmarks/` can write results as CSV and run the same measurements against several environments (`--env current --env 'ngio==1.0.0,zarr==3.0.6'`), each isolated via `uv`, printing a comparison with ratios. `current` means the working tree, so the effect of an optimization is measurable before it is published. Any requirement can be pinned, not just ngio: installing an older ngio also resolves *its* dependencies, so a difference between versions can come from zarr instead, and pinning both is the only way to tell. Fixture data is generated through ngio's public API alone, which is what lets a block run unmodified against an older ngio; it is seeded and tuned to compress at roughly the rate of a real sample image, since uniform data would silently render the chunk-shape and codec comparisons meaningless.
- `benchmarks/` takes `--config FILE`, a TOML experiment: which blocks, which axis values, which environments, where the CSV goes. An invocation is an experiment, and one that lives only in shell history cannot be committed beside the results it produced or diffed against last month's. A config can only *replace* an axis a block already declares, never add one, and it lowers to exactly the same override a `--axis` flag produces — so the file and the flag cannot drift, and the CSV columns stay a literal. Every axis entry names its block (`[axes.consolidate]`); the unqualified `--axis z=16` the command line takes has no config equivalent, because a file is read later by someone who was not there and "whichever blocks happen to have a `z`" describes the suite rather than the experiment. The command line beats the file, with `--no-keep`/`--no-quiet` added so a boolean set in a file can still be switched off. `--list` applies the config, making it a dry run of the experiment.
- `benchmarks/` measures wall clock and peak allocation in two separate phases. `tracemalloc` hooks every allocation and inflates allocation-heavy code, and it does not inflate every case equally — the pyramid mode that allocates most was penalised most, which is exactly the comparison being made, so the timings understated `numpy`'s advantage over `dask` (~2.4x, not the ~3x measured under instrumentation). Peak is now the max over its runs rather than whatever the last one happened to be.
