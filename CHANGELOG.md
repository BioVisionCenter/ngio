...
- **Iterators run at scale, under one verb table**: every iterator has a topic verb (`process`, `segment`, `measure`, `detect`) over the shared generic layer (`map`, `reduce`, `iter`); work fans out on threads or processes (`ThreadedMapper`/`ProcessMapper`, lock-free via conflict-free waves), reads context with `with_halo(...)` at unchanged parallelism, batches patches for neural networks (`BatchedMapper`, `iter(batch_size=...)`), and distributes across SLURM/Fractal tasks with the one `prepare_jobs → for_job → finalize` recipe.
...
- Topic verbs: `process` (image processing) and `segment` (segmentation, masked included) are the partition-aware domain spellings of `map`; `measure` and `detect` complete the table on the read-only iterators. One per iterator, `finalize()` the one gather everywhere.
- One reconciliation declaration per iterator, in the builder chain: `on_overlap(policy)` on the writers (contested write pixels: `"last"` for wave order, or any `merge=` rule), `with_stitch(config)` on segmentation, `with_join(join)` on features, `with_nms(nms)` on detection — each backed by a swappable protocol (`SeamMatcherProtocol`/`IouSeamMatcher`, `JoinProtocol`/`ConcatJoin`, `NmsProtocol`/`GreedyNms`).
- `BatchedMapper(batch_size=...)` stacks patches into one `(B, ...)` array per `func` call, for neural-network inference; `iter(batch_size=...)` is the manual batched loop (numpy-only and eager — combining it with `lazy=True` or `data_mode="dask"` raises).
- `reduce(func, mapper=...)` returns per-ROI results, writing nothing.
- `with_halo(x=, y=, ...)` reads a grown region and writes only the core; write footprints — and so parallelism — are unchanged, and misuse refuses loudly. On the read-only iterators the halo is a pure read margin: detection reconciles the overlap with NMS, feature extraction hands the reconciliation to your declared join.
- `with_stitch(...)` (declared in the builder chain) merges objects split across region boundaries into one id, on any ROI list — grids with a halo, overlapping FOVs, ragged tables — compacting ids to a dense `1..N`; an interrupted run leaves a valid, merely over-split label. On `MaskedSegmentationIterator` it merges sub-objects within one mask (no `UniqueLabelsTransform` needed; combining raises). The seam decision is a swappable protocol: `StitchConfig(seam_matcher=...)` takes any `(patch_a, patch_b) -> [(id_a, id_b), ...]` criterion (`SeamMatcherProtocol`), `IouSeamMatcher` being the default.
- Distributed runs: `prepare_jobs(n_jobs)` → `for_job(**args).<topic verb>(func)` per SLURM/Fractal task → one gather, `finalize()`, on every iterator (on the read-only ones it merges the banked partials and returns the table; on a slice `measure`/`detect` bank and return `None`). Partitions never share a write unit, every step validates a plan fingerprint, and the result matches a serial run.
- `ObjectDetectionIterator.detect(func)` ([#111](https://github.com/BioVisionCenter/ngio/issues/111)) runs a `(patch) -> list[Roi]` detector per tile, anchors the boxes to world coordinates, deduplicates by the declared NMS (`with_nms(GreedyNms(...))` — or anything satisfying `NmsProtocol` over the public `Detection` records: soft-NMS, class-aware suppression), and returns one renumbered `RoiTable`; `Roi.anchor` and `bbox_iou` are public.
- `FeatureExtractorIterator.measure` joins per-ROI `func(image, label, roi)` results into one `FeatureTable` via the declared join (`with_join(...)` — any `JoinProtocol` callable; `ConcatJoin` is the default). Every result row is stamped with `roi_index`/`roi_name` provenance columns (kept in the default table), `with_halo(...)` reads context around each region, and a border object measured by several grown regions is deduplicated in your declared join via those columns.
...
- **(beta-only break)** The distributed read-only verbs collapsed into the topic verbs: `measure_to_partial`, `detect_to_partial`, and `merge_partials` (shipped only in 1.1.0 betas) are gone. On a `for_job` slice `measure`/`detect` bank the partial and return `None` (their return types are now `Table | None` / `RoiTable | None`); the gather is `finalize()`, which on the read-only iterators returns the table and raises on any slice, when nothing was banked, or — for a slice's `measure` — when handed a `coalesce` (the join is global: pass it to `finalize(coalesce=...)`). `iter_batched` folded into `iter(batch_size=...)` the same way, and `AbstractIteratorBuilder` gained a third Generic parameter (the `finalize` result), so external subscriptions need one more argument.
- **(beta-only break)** Reconciliation moved into the builder chain: the `stitch=`/`nms=` constructor arguments and the `measure`/`finalize` `coalesce=` kwargs are gone — declare `with_stitch(...)`/`with_nms(...)`/`with_join(...)` before `for_job` (a slice's declared join is inert; the old slice-coalesce refusal is gone). `NmsConfig` is renamed `GreedyNms` (it implements the suppression, so "Config" was a misnomer).
- **(beta-only break)** Segmentation refuses undeclared overlapping write footprints: without `with_stitch` or `on_overlap`, overlapping label writes raise at every writing verb instead of silently last-writer-winning. The check is pixel-exact (a halo never triggers it; sharing a chunk without sharing pixels passes); masked writes are exempt by construction; image processing keeps the permissive wave-order default.
- **(beta-only break)** A serial `measure`'s declared join now receives the normalized list the distributed gather always produced — DataFrames with a `label` column, stamped `roi_index`/`roi_name`, empty ROIs as empty column-less frames — instead of the function's raw returns. Functions returning a `_ngio_index`, `roi_index`, or `roi_name` column are refused, and the default `measure` table gains the two provenance columns.
