...
- Added a performance gate at `tests/performance/`, asserting exact store-operation counts against committed baselines and running in CI like any other test: ngio's regressions are algorithmic, so they are exact integers with zero variance, and a change that multiplies store reads fails the build with a readable before/after diff. Counts are backend-independent, so a local measurement predicts the cost on S3 without any credentials in CI — checked rather than assumed, by running every scenario against both a local and an in-memory store. There is no history file: the baselines are committed JSON, so `git log -p` on them already is the history. Gated operations: creating and opening a container, creating a plate, enumerating a plate's wells and images, aggregating tables across a plate, loading tables, reading whole arrays and ROIs, and pyramid consolidation in all three modes.
