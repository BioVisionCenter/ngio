...
- `with_halo(x=, y=, ...)` reads a grown region and writes only the core; write footprints — and so parallelism — are unchanged, and misuse refuses loudly. On the read-only iterators the halo is a pure read margin: detection reconciles the overlap with NMS, feature extraction hands the reconciliation to your `coalesce`.
...
- `FeatureExtractorIterator.measure` joins per-ROI `func(image, label, roi)` results into one `FeatureTable` (custom `coalesce` supported). Every result row is stamped with `roi_index`/`roi_name` provenance columns (kept in the default table), `with_halo(...)` reads context around each region, and a border object measured by several grown regions is deduplicated in your `coalesce` via those columns.
...
- **(beta-only break)** A serial `measure`'s custom `coalesce` now receives the normalized list the distributed gather always produced — DataFrames with a `label` column, stamped `roi_index`/`roi_name`, empty ROIs as empty column-less frames — instead of the function's raw returns. Functions returning a `_ngio_index`, `roi_index`, or `roi_name` column are refused, and the default `measure` table gains the two provenance columns.
