...
- **Iterators gained `with_halo(x=, y=, z=, t=)`: read a margin of context around each ROI, write only the ROI.** The function is handed the grown region and must return it grown too; the border is cropped off before the write. That is the fix for seam artifacts — a smoothing or a segmentation sees past its own tile edge — and it is also "trim", which is the same operation viewed from the other side. Crucially the ROIs do not move, so **write footprints are unchanged and a haloed iterator parallelizes exactly as far as it did without one**: overlapping *writes* would have to be serialized, overlapping reads cost nothing. Margins are in reference-image pixels and clip at the borders, so an edge tile grows only on the sides where there is room and the crop is asymmetric to match. The crop is a setter wrapper rather than a transform, deliberately: writing folds the transform chain outermost-first, so a terminal crop transform would run *after* a read-modify-write transform's read-back and hand it a still-grown patch — keeping it outside the chain sidesteps the ordering and leaves the one-terminal-read-modify-write slot free for a mask or a merge. Returning a core-sized array raises rather than writing a shifted tile. Read-only iterators refuse the call: with no written region there is nothing for a halo to surround. So does a **shape-changing transform on the data path** — a `ZoomTransform` round-trip (downsample for the model, upsample to write) works fine on its own, but its patch lives on a rescaled grid where the reference-pixel margins no longer describe the border, and `ZoomTransform`'s inverse tolerates a pixel of slack, so a crop at the wrong scale would be quietly resampled into place rather than caught. Refused rather than guessed at; iterate on a coarser pyramid level to work at a lower resolution. `MaskTransform` is unaffected and composes with a halo normally — its zoom rescales the *label* it reads, never the data array.
- **`ngio.transforms.MergeTransform` — a write can now combine with what is already on disk instead of overwriting it.** `image.set_roi(roi, patch, transforms=[MergeTransform("max")])` reads the destination region back, folds it with the patch, and writes the result; `"min"`, `"sum"` and `"keep_nonzero"` ship alongside, or pass your own `(existing, patch, ctx) -> array`. `"max"`, `"min"` and `"sum"` are commutative and associative, so overlapping regions give the same answer whatever order they are written in; `"keep_nonzero"` is "the last nonzero write wins" and, like a custom rule, depends on the visit order where regions overlap. Deliberately no feathering: correct mean-blending needs an accumulated per-pixel weight, and the naive `w*patch + (1-w)*existing` darkens the first-written edge of every tile, so it would be a trap rather than a feature.
- **Reading the destination on the write path is now a contract, not a one-off.** `MaskTransform` was quietly the first transform to do it; the behaviour now lives in a `ReadModifyWriteTransform` base (exported from `ngio.transforms`) that both it and `MergeTransform` sit on, and subclasses implement one method, `reconcile(existing, patch, ctx)`. Three rules come with it. Such a transform must be **last** in the chain, because the read-back replays the transforms placed *before* it and anything after would be skipped during that read. There may be **at most one** per chain — writing folds the chain outermost-first, so a second one would read the destination again and merge an already-merged array; the pipes now raise `NgioValueError` naming both rather than producing that. And it must **never read outside its own write footprint**: reading exactly `ctx.slicing` is what keeps this safe under `ThreadedMapper` and `ProcessMapper`, which already refuse two ROIs sharing a write unit and thereby give each unit sole ownership of the pixels it reads. Read-modify-write therefore forbids nothing that parallel mapping did not already forbid. The pipe also **binds the preceding chain itself** now, which removes a footgun `MaskTransform` shipped with: the caller used to have to hand the transform a second copy of that chain (`transforms=[A, mask]` *and* `set_transforms=[A]`), and forgetting it made the read-back silently skip `A` and merge two arrays taken from different points in the chain. `set_transforms=` survives as an explicit override for a pipe-less caller and still wins where it is given, so nothing written against it changes; it is simply no longer something you can forget. Binding produces a *copy* of the transform rather than mutating it, so one instance stays reusable across pipes with different chains — the reuse the mask docstring already promised across ROIs.
- **`ngio.transforms.MaskTransform` — masking is now a transform, not four classes.** One backend-dispatching implementation replaces the numpy/dask masked-pipe duplication, and masking composes with everything the pipes already do: `image.get_roi(roi, transforms=[MaskTransform(label=nuclei, target_image=image)])` reads a masked patch, `set_roi` with the same transform write-protects the pixels outside the mask by merging the patch with the on-disk data. The ROI comes from the call's context — `label == roi.label` selects the mask, `label != 0` for an unlabelled ROI — so a single instance is reusable across ROIs; a label at a different pyramid level is rescaled to the data grid (nearest) unless `allow_rescaling=False`; `fill_value` applies on read only. It must be the last transform in the chain, and the only read-modify-write one — see the `MergeTransform` entry below for why, and for the contract it now shares.
