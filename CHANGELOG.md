...
- `ThreadedMapper` and `ProcessMapper` parallelize `map`/`reduce`. Writes run in conflict-free waves, and wave order is the canonical write order for every mapper — serial included — so overlapping writes land identically everywhere; `write_order="roi"` additionally makes the later ROI win contested pixels, manual `iter` loops included. The default stays serial.
...
- `write_order=` on `on_overlap(...)` and `StitchConfig` — performance first, reproducibility opt-in, exactness always. `"any"` (the default) schedules pixel-contested writes for parallelism alone: exactness and per-version determinism kept, seam ownership schedule-defined, 1.5–2.5x faster on parallel mappers over overlapping tilings. `"roi"` opts into the later-ROI-wins order: reproducible seams, `map` bit-identical to the manual `iter` loop. A pair relaxes only when both sides declare `"any"`; masked segmentation always schedules freely (mask-protected writes never contest).
...
- `ObjectDetectionIterator.detect(func)` ([#111](https://github.com/BioVisionCenter/ngio/issues/111)) runs a `(patch) -> list[Roi]` detector per tile, anchors the boxes to world coordinates, deduplicates by the declared NMS, and returns one renumbered `RoiTable`; `Roi.anchor`, `Detection`, and `bbox_iou` are public.
...
- The `io_retry.retry_all_errors` warning fires once when the config loads, not from the model validator — which re-ran on every `model_copy`/assignment and raised in consumer suites running `-W error`.
- Overriding a roi-pinned axis with a slicing kwarg drops the pipe's `roi`: it no longer described the effective region, so anything trusting it (a mask transform, world anchoring) now refuses instead of misplacing pixels. Same-axis overrides on `*_masked` reads/writes refuse for the same reason (they were only correct when the mask shared the data's resolution). The roi/kwarg hierarchy is now documented on every public method that accepts both.
- Pickling a `StitchPlan` no longer creates the scratch group on the store as a side effect: the writing verbs resolve the scratch up front in the parent, and pickling an unresolved plan raises instead of silently writing.
...
- Serial `map` over overlapping write footprints runs in canonical wave order — "who wrote last" is identical under every mapper; disjoint tilings are unaffected.
