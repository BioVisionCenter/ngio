...
- **`set_space_unit` / `set_time_unit`** — set one unit without touching the other, on images, labels, the images container and `OmeZarrContainer` (with `set_labels=` to carry the unit into the labels). The replacement for the deprecated batch `set_axes_units`; see Deprecated.
